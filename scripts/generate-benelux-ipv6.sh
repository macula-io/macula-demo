#!/bin/bash
# Generate BeNeLux relay identities with per-identity IPv6 addresses.
#
# Format: hostname/city/country/lat/lng/ipv6_addr
# IPv6 addresses are sequential from the box's /64 range, starting at ::100.
#
# Usage:
#   ./scripts/generate-benelux-ipv6.sh                    # with IPv6
#   ./scripts/generate-benelux-ipv6.sh --no-ipv6          # without (legacy compat)
#   ./scripts/generate-benelux-ipv6.sh --prefix 2600:3c0e:e001:ec  # custom prefix
#
...
emit() {
  ...
  if [ "${NO_IPV6}" = "--no-ipv6" ]; then
    echo "${host}/${city}/${country}/${lat}/${lng}"
  else
    local hex
    hex=$(printf '%x' $INDEX)
    echo "${host}/${city}/${country}/${lat}/${lng}/${IPV6_PREFIX}::${hex}"
    INDEX=$((INDEX + 1))
  fi
}
...
echo "# BeNeLux relay identities: ${TOTAL} cities (BE:${#BE[@]} NL:${#NL[@]} LU:${#LU[@]})" >&2
if [ "${NO_IPV6}" != "--no-ipv6" ]; then
  echo "# IPv6 prefix: ${IPV6_PREFIX}::/64, range ::100-::$(printf '%x' $((256 + TOTAL - 1)))" >&2
fi
echo "" >&2
...
for entry in "${ALL[@]}"; do
  ...
  if [ "${NO_IPV6}" = "--no-ipv6" ]; then
    IDENTITY="${host}/${city}/${country}/${lat}/${lng}"
  else
    hex=$(printf '%x' $INDEX)
